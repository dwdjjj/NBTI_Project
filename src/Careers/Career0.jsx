...
export default function Carrer2() {
  ...
  async function putData1(url) {
    try {
      ...
      const response = await axios.post(
        `https://2rksqj0zwg.execute-api.ap-northeast-2.amazonaws.com/dev/youtube/0/${url.replaceAll(
          "/",
          "$"
        )}`
      );
  ...
  async function putData2(url) {
    try {
      //응답 성공
      const response = await axios.post(
        `https://2rksqj0zwg.execute-api.ap-northeast-2.amazonaws.com/dev/blog/0/${url.replaceAll(
          "/",
          "$"
        )}`
      );
        ...
        {crr_link1.map((s, i) => {
          return (
            <Card>
              <span>
                {" "}
                {i + 1}번째 유용한 유튜브 영상입니다. 링크는 영상 아래에!{" "}
              </span>
              <iframe
                width="560"
                // width="100%"
