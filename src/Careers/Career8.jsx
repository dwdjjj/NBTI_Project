...
export default function Carrer2() {
  ...
  async function putData1(url) {
    try {
      ...
      const response = await axios.post(
        `https://2rksqj0zwg.execute-api.ap-northeast-2.amazonaws.com/dev/youtube/8/${url.replaceAll(
          "/",
          "$"
        )}`
      );
  ...
  async function putData2(url) {
    try {
      //응답 성공
      const response = await axios.post(
        `https://2rksqj0zwg.execute-api.ap-northeast-2.amazonaws.com/dev/blog/8/${url.replaceAll(
          "/",
          "$"
        )}`
      );
